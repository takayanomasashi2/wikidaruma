...
const TailwindAdvancedEditor: FC<TailwindAdvancedEditorProps> = ({
 ...
 const extractBlockContent = (node: any): string => {
   ...
   if (Array.isArray(node.content)) return node.content.map(extractBlockContent).join('');
   return '';
 };
...
const processBlocks = async (editor: EditorInstance) => {
 ...
 const existingBlocks = await fetch(`/api/blocks?pageId=${pageId}`).then(r => r.json()) as ApiBlock[];
 const existingBlockIds = new Map(existingBlocks.map(b => [b.order, b.id]));
...
const getNodeContent = (node: any): string => {
 ...
 const processNode = async (node: any, parentOrder: number) => {
   ...
   if (node.type === 'heading' && node.attrs?.level) {
     ...
     switch(node.type) {
       case 'bulletList': blockType = 'bulletList'; break;
       case 'taskList': blockType = 'taskList'; break;
       case 'orderedList': blockType = 'orderedList'; break;
       case 'listItem': blockType = 'listItem'; break;
       case 'horizontalRule': blockType = 'horizontalRule'; break;
       case 'math': blockType = 'math'; break;
       case 'twitter': blockType = 'twitter'; break;
       case 'codeBlock': blockType = 'codeBlock'; break;
       case 'paragraph': blockType = 'paragraph'; break;
       case 'doc': blockType = 'doc'; break;
       default: break;
     }
   }
